...
const transform: AxiosTransform = {
    ...
    transformRequestHook: (res: AxiosResponse<Result>, options: RequestOptions) => {
        ...
        const { success, msg, records, total, code } = data
        const resData = data.data

        // 列表接口返回的model 用 success 判断是否成功 ， 非列表请求 用 code 判断是否成功
        // Reflect.has(data, 'success') &&
        // Reflect.has(data, 'code') &&
        const hasSuccess = (data && has(data, 'success') && success) || (data && has(data, 'code') && code === ResultEnum.SUCCESS)
        ...
        if (code === ResultEnum.SUCCESS || success) {
            if (code === ResultEnum.SUCCESS) return resData

            // 如果是列表请求成功， 返回 总数量以及数据
            return {
                records,
                total
            }
        ...
        if (code === ResultEnum.ERROR) {
            if (msg) {
                ...
                ElMessage.error('sys.api.errorMessage')
    ...
    responseInterceptorsCatch: (error: any) => {
        ...
        try {
            if (code === 'ECONNABORTED' && message.indexOf('timeout') !== -1) {
                ElMessage.error('sys.api.apiTimeoutMessage')
            }
            if (err?.includes('Network Error')) {
                ElMessageBox({
                    title: 'sys.api.networkException',
                    message: 'sys.api.networkExceptionMsg'
                })
            }
        } catch (error) {
